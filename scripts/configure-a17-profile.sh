...
for symbol in \
  ...
  NETFILTER NF_CONNTRACK NF_NAT NETFILTER_XTABLES NETFILTER_XT_MARK \
  NETFILTER_XT_MATCH_ADDRTYPE NETFILTER_XT_MATCH_CONNTRACK \
  NETFILTER_XT_MATCH_COMMENT NETFILTER_XT_MATCH_MULTIPORT \
  NETFILTER_XT_MATCH_IPVS NETFILTER_XT_TARGET_MASQUERADE \
  IP_NF_IPTABLES IP_NF_FILTER IP_NF_MANGLE IP_NF_RAW \
...
for symbol in \
  VIRTUALIZATION KVM VSOCKETS VHOST VHOST_NET VHOST_VSOCK \
...
grep -E '^(CONFIG_(NAMESPACES|UTS_NS|IPC_NS|PID_NS|NET_NS|CGROUPS|CGROUP_DEVICE|CGROUP_PIDS|MEMCG|SECCOMP_FILTER|VETH|BRIDGE|OVERLAY_FS|VIRTUALIZATION|KVM|VHOST_NET|VHOST_VSOCK)=|# CONFIG_FUNCTION_TRACER is not set)' \
