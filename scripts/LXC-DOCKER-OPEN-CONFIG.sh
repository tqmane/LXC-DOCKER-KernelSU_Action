...
ered() { echo -e "\033[31m" "$@"; }
egreen() { echo -e "\033[32m" "$@"; }
ewhite() { echo -e "\033[37m" "$@"; }

echo -e "\n\nChecking config file for Android LXC/Docker config options.\n\n"

errors=0
fixes=0

for c in $CONFIGS_ON $CONFIGS_OFF; do
	cnt=$(grep -w -c "$c" "$FILE" || true)
	if [ "$cnt" -gt 1 ]; then
...
if [ "$fixes" -gt 0 ]; then
	egreen "Enabled/fixed $fixes Docker/LXC config entries.\n\n"
fi
